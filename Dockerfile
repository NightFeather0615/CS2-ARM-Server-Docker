...
ENV MAX_PLAYER="16"
ENV CS_ARGS="+bot_quota 0 +game_mode 1 +map de_mirage"
...
RUN apt update && apt install -y git cmake python3 wget libdbus-1-3 libsdl3-0
...
WORKDIR /root/Steam/steamapps/common/Counter-Strike\ Global\ Offensive/game/bin/linuxsteamrt64

CMD cd /root/Steam/steamapps/common/Counter-Strike\ Global\ Offensive/game/bin/linuxsteamrt64 && \
    bash /steamcmd/steamcmd.sh +login anonymous +app_update 730 validate +quit && \
    BOX64_ADDLIBS=libv8_libcpp.so:libtier0.so:/steamcmd/linux64/steamclient.so \
    ./cs2 -dedicated -usercon -maxplayers_override ${MAX_PLAYER} +rcon_password ${RCON_PASSWD} ${CS_ARGS}
